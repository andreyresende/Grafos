...
void Graph::longestPath() {
  ...
  for (auto & i : this->vertexes) {//Coloca na fila as materias iniciais que serao pre requisito de outras
    if (i.getGrau() == 0) {
      if (i.adjacents.size() != 0) {
        cout << i.getElement() << ": " << i.getName() << " tem " << i.adjacents.size() <<" vizinhos" << endl;
        fila.push(i);
      }
    }
  }
  cout << endl << endl << endl << endl;
  while (fila.size() > 0) {
    int distancia[fila.front().adjacents.size()];
    cout << "Distancia.size = " << fila.front().adjacents.size() << endl;
    Vertex vertice = fila.front();
    fila.pop();
    int count = 0;
    for (auto & i : vertice.adjacents) {
      distancia[count] = this->max(distancia[count], vertice.getWeight());
